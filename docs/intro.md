---
sidebar_position: 1
slug: /
---

# Lista Recurring Events

**Powerful recurring events for WordPress with Bricks Builder & Elementor integration.**

Lista Recurring Events transforms your WordPress site into a full-featured event management platform. Create events once and let the plugin automatically calculate and display all future occurrences—no need to create multiple posts for repeating events.

![Plugin Overview](/img/lista-recurring-events-ui-calendar.webp)

## Key Features

- **Smart Recurrence Calculation** — Define patterns once; occurrences are calculated dynamically
- **Multiple Recurrence Patterns** — Daily, Weekly, Monthly (by weekday or date), Yearly, and Custom dates
- **Bricks Builder Integration** — Custom query loop, dynamic data tags, and filter elements
- **Elementor Integration** — Loop widget, dynamic tags, and filter widgets
- **Interactive Calendar** — Beautiful FullCalendar integration with the `[lre_calendar]` shortcode
- **Add to Calendar** — Let visitors save events to Google Calendar, Apple Calendar, Outlook, and more
- **Field Overrides** — Set different values (title, image, location) for specific occurrences
- **Multi-Post Type Support** — Configure multiple custom post types as event types
- **Performance Optimized** — Intelligent caching prevents redundant calculations

## How It Works

Traditional event plugins require you to create a separate post for each occurrence of a repeating event. Lista Recurring Events takes a different approach:

1. **One Post Per Event** — Create a single event post with your content
2. **Set the Pattern** — Configure how often the event repeats
3. **Automatic Calculation** — The plugin calculates all future occurrences on-the-fly
4. **Dynamic Display** — Show upcoming dates using dynamic tags, shortcodes, or query loops

This approach keeps your database clean, makes updates simple (change once, update everywhere), and provides flexibility for complex scheduling needs.

## Requirements

| Requirement | Version |
|------------|---------|
| WordPress | 6.8 or higher |
| PHP | 8.1 or higher |
| Page Builder | Bricks Builder or Elementor (optional) |

## Free vs Pro Features

Lista Recurring Events offers both free and licensed functionality:

### Free Features
- Daily recurrence pattern
- Single occurrence display
- Basic shortcodes
- Calendar display (without filtering)

### Pro Features (License Required)
- Weekly, Monthly, Yearly, and Custom Date patterns
- Multiple occurrences display
- Bricks Builder query loop integration
- Elementor Pro query loop integration
- AJAX filtering
- Full dynamic data tags
- Field overrides per occurrence

[Purchase a license →](https://listapage.au/custom-wordpress-plugins/recurring-events/)

## Quick Start

Ready to get started? Follow these steps:

1. [Install the plugin](./getting-started/installation)
2. [Configure your settings](./getting-started/configuration)
3. [Create your first recurring event](./getting-started/first-event)

---

**Need help?** Check the [Troubleshooting](./troubleshooting/common-issues) section or contact support at [listapage.au](https://listapage.au).
